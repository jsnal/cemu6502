...
/* 0x00            0x01  0x02            0x03  0x04                0x05                0x06                0x07  0x08  0x09            0x0A         0x0B  0x0C            0x0D            0x0E            0x0F*/
   ...
   fail,           fail, fail,           fail, handler_ld_zpg_idx, handler_ld_zpg_idx, handler_ld_zpg_idx, fail, fail, fail,           fail,        fail, fail,           fail,           fail,           fail, /* 0xB0 */
...
{
  assert_or_fatal(REG1 != NULL);
  uint8_t address = get_abs_address(params->machine->memory, params->machine->cpu);
  *REG1 = memory_get_byte(params->machine->memory, address);
  set_zero_and_negative_flags(params->machine->cpu, *REG1);
}
